...
class FormatsTestCase(TestCase):
    ...
    def test_video_format(self):
        ...
        if re.search("3D",program,re.I):
            self.assertEquals(timing_info, video_golden_timing[self.format], "Video timing should be the same with golden value")
        else:    
            actual_timing = trans_info(timing_info.split())
            golden_timing = trans_info(video_golden_timing[self.format].split())
            ...
            for i in range(3,14):
                if  i == 6 or i == 11:
                    ...
                    if actual_timing[i] == golden_timing[i]:
                        self.assertTrue(True, "Actual "+ timing_info.split()[i] +",  Golden "+video_golden_timing[self.format].split()[i])
                    elif (actual_timing[i] < golden_timing[i] + 1) and (actual_timing[i] > golden_timing[i] - 1):
                        self.assertTrue(False, "Actual "+ timing_info.split()[i] +",  Golden "+video_golden_timing[self.format].split()[i],iswarning=True)
                    else:
                        self.assertTrue(False,"Actual "+ timing_info.split()[i] +",  Golden "+video_golden_timing[self.format].split()[i])
                elif i==13:
                    # TMDS clock
                    if actual_timing[i] == golden_timing[i]:
                        self.assertTrue(True, "Actual "+ timing_info.split()[i] +",  Golden "+video_golden_timing[self.format].split()[i])
    ...
    def test_audio_format(self):
        ...
        try:
            ...
            audio_channel_number, audio_frequency, audio_size = self.format.split('_')
            print video_program
            with self.device.log_subject.listen(self.listen_keyword) as listener:
            ...
            self.assertEquals(timing_info, video_golden_timing[self.format], "Audio Info frame should be the same with golden value")
