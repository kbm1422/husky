...
##            version = 0
    ...
    def getComPortNumber(self):
        if sys.platform == 'win32':
            ...
            dll = c.CDLL('libftd2xx.so')
        port = _ft.LONG()
        call_ft(dll.FT_GetComPortNumber, self.handle, c.byref(port))
        return port.value
...
def listComPorts():
    ...
    if nums is not None:
        for num in nums:
            dev = openEx(num)
            try:
                port = dev.getComPortNumber()
                comports.append("COM" + port)
            finally:
                dev.close()
    print comports

__all__ = ['call_ft', 'listDevices', 'listComPorts', 'getLibraryVersion',
