...
from simg.devadapter.logsubject import FileLogSubject, SerialLogSubject
...
class BostonChipAdapter(BaseDeviceAdapter):
    def __init__(self, conn, comport=None, **kwargs):
        super(BostonChipAdapter, self).__init__(**kwargs)
        self.__conn = conn
    ...
    def open(self):
        if self.__comport:
            self.log_subject = SerialLogSubject(self.__comport, 115200)
            self.log_subject.open()
    ...
    def sendcmd(self, cmd, timeout=5.0, interval=0.5, trim=True, wakeup=True):
        ...
        if "Warning:BB is in Sleep Mode" in resp and wakeup is True:
            logger.debug("Module is in sleep mode, force to wake up it")
            self.__conn.sendcmd("pwr_set_mode 3 0")
            logger.debug("retry cmd: %s", cmd)
            resp = self.__conn.sendcmd(cmd, timeout, interval, trim)
        return resp
...
class JaxDeviceAdapter(BaseDeviceAdapter):
    def __init__(self, moduleid, comport=None, tcpport=None, logname=None, name="", **kwargs):
        ...
        self.boston = BostonChipAdapter(self.conn, comport)
    ...
    def set_ss_server_logname(self, newname):
        ...
        self.boston.log_subject.logname = "%s_Boston%s" % (os.path.splitext(newname))
        self.gen3_1.open()
        self.gen3_2.open()
        time.sleep(3)
