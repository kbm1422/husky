...
from .mark import SkipIfDecorator, ParametrizeDecorator, NameDecorator
...
class TestCase(unittest.TestCase):
    ...
    def run(self, result=None):
        ...
        try:
            mark = self._get_name_mark()
            if mark is not None and mark.name is not None:
                self.name = mark.name % self.__dict__
        ...
        except (SyntaxError, AttributeError, ValueError, IndexError):
    ...
    def get_class_parametrize_marks(cls):
        marks = OrderedDict()
        if hasattr(cls, ParametrizeDecorator.MARK_NAME):
            marks.update(getattr(cls, ParametrizeDecorator.MARK_NAME))
        return marks
    ...
    def get_method_parametrize_marks(cls, method_name):
        ...
        if hasattr(method, ParametrizeDecorator.MARK_NAME):
            marks.update(getattr(method, ParametrizeDecorator.MARK_NAME))
        return marks

    def eval_parametrize_attrs(self, fetch=None):
        for mark in self._parametrize_marks.values():
            if not hasattr(self, mark.name):
                if mark.default is not None:
                    setattr(self, mark.name, mark.default)
                else:
                    raise AttributeError("The attribute '%s' is required for %s" % (mark.name, self))
    ...
    def _cast_parametrize_attrs(self, mark):
        ...
        attr_value = getattr(self, mark.name)
        if not isinstance(attr_value, mark.type):
            if mark.type is types.BooleanType:
                setattr(self, mark.name, sstring.get_boolean(attr_value))
            elif mark.type is types.FloatType:
                setattr(self, mark.name, mark.type(attr_value))
            else:
                ...
                if not isinstance(new_attr_value, mark.type):
                    ...
                    setattr(self, mark.name, new_attr_value)

    def _get_name_mark(self):
        method = getattr(self, self._testMethodName)
        return getattr(method, NameDecorator.MARK_NAME, None) or getattr(self, NameDecorator.MARK_NAME, None)
...
class LinkedTestCase(TestCase):
    ...
    def _get_name_mark(self):
        return getattr(self, NameDecorator.MARK_NAME, None)
