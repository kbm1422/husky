...
class TestService(object):
    ...
    XMLRPC_BENCHES_PATH = "/husky/xmlrpc/testbenches"
    ...
    def newTestRunner(self, kwargs, feedback=True):
        ...
        uid = str(kwargs["uid"])
        if feedback:
            raise NotImplementedError("if feedback is true, send record to RabbitMQ")
            # url = urljoin('http://%s:%s' % (self._cavia.host, self._cavia.port), self._cavia.root)
            # url = urljoin(url, 'testexecutions/%s/testrecords' % uid)
            # handler = HttpCaseRecordHandler(url)
            # result = TestResult()
            # result.addCaseRecordHandler(handler)
    ...
    def start(self):
        ...
        runners_dispatcher = XMLRPCDispatcher(allow_none=True)
        runners_dispatcher.register_function(self.newTestRunner)
        runners_dispatcher.register_function(self.delTestRunner)
        self._rpcserver.add_dispatcher(self.XMLRPC_RUNNERS_PATH, runners_dispatcher)

        benches_dispatcher = XMLRPCDispatcher(allow_none=True)
        benches_dispatcher.register_function(lambda: TestLoop().get_all_resource_states())
        self._rpcserver.add_dispatcher(self.XMLRPC_BENCHES_PATH, benches_dispatcher)
